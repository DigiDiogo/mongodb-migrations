...
(function() {
  ...
  Migrator = (function() {
    function Migrator(dbConfig, logFn) {
      ...
      this._latesCollName = dbConfig.latestCollection || '_latestMigrations';
    ...
    Migrator.prototype._latestColl = function () {
      return this._db.collection(this._latesCollName);
    };

    Migrator.prototype.saveLatestMigrations = function(migrations) {
      return this._latestColl().update(
        {_id: 'latestMigrations'},
        {_id: 'latestMigrations', migrations : migrations},
        {upsert: true});
    };

    Migrator.prototype.removeLatestMigrations = function() {
      return this._latestColl().deleteOne({_id: 'latestMigrations'});
    };

    Migrator.prototype.getLatestMigrations = function() {
      return this._latestColl().find({_id: 'latestMigrations'}).toArray().then(function(result){
        return result[0].migrations;
      });
    };
    ...
    Migrator.prototype._runWhenReady = function(direction, cb, progress) {
      ...
      allDone = (function(_this) {
        return function(err) {
          const promises = migrationsCollectionUpdatePromises;
          if(direction === 'up' || direction === 'forceDown'){
            const migrationsId = _.map(_this._result, function(migration, keyId){ return keyId;});
            if(direction === 'up') {
              promises.push(_this.saveLatestMigrations(migrationsId));
            } else {
              promises.push(_this.removeLatestMigrations());
            }
          }
    ...
    Migrator.prototype.forceRollback = function(done, progress) {
      var _this = this;
      this._dbReady.then(function() {
        _this.getLatestMigrations().then(function(migrations){
          _this._m = _.filter(_this._m, function(migration) {
            return _.indexOf(migrations, migration.id) > -1;
          });
          _this._runWhenReady('forceDown', done, progress);
        });
      });
    };
